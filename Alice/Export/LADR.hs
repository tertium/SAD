{-
 -  Export/LADR.hs -- print proof task in LADR syntax
...
module Alice.Export.LADR (ladrOut) where
...
ladrOut :: Prover -> Int -> [Context] -> Context -> String
ladrOut pr tl cn gl = (hdr . sos . axs . cnj . eol) ""
  where
    ...
    axs = foldr (flip (.) . ladrForm . cnForm) id cn
    cnj = ladrForm (Not $ cnForm gl)

    sos = showString "formulas(sos).\n"
    eol = showString "end_of_list.\n"


-- Formula print

ladrForm :: Formula -> ShowS
ladrForm f = ladrTerm 0 f . showString ".\n"

ladrTerm :: Int -> Formula -> ShowS
ladrTerm d = dive
  where
    ...
    binder f  = ladrTerm (succ d) (Ind 0 []) . showChar ' '
              . ladrTerm (succ d) f
